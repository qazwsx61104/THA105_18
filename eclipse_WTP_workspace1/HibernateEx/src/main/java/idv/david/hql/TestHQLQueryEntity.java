...
public class TestHQLQueryEntity {
	...
	public static void main(String[] args) {
		...
		try {
			...
			List<Emp> list2 = session.createQuery("from Emp").list(); //黃色警告是因為沒有加上Emp.class
			
			List<Emp> list3 = session.createQuery("from Emp order by empno", Emp.class).list(); // empno 是屬性名稱 , 非SQL欄位
			...
			List<Emp> list6 = session.createQuery("from entity.Emp", Emp.class).list(); //套件名稱.class
